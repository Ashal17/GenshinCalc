...
if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true){
	...
	if($stmt = mysqli_prepare($link, $sql)){
		...
		if (mysqli_stmt_execute($stmt)){
			...
			//update
				$sql = "UPDATE genshin_user_data SET artifact_list = ? WHERE id = ?";
			}else{
			//insert
				$sql = "INSERT INTO genshin_user_data(artifact_list, id, setup) VALUES (?, ?, NULL)";
			}
			if($stmt = mysqli_prepare($link, $sql)){
				...
				if (mysqli_stmt_execute($stmt)){
					...
					$errormessage = "Error when updating Artifact list";
				}
			}else{		
				$errormessage = "Error when updating Artifact list";
			}
